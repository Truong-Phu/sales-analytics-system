...
export default function OrdersPage() {
    ...
    // Mở modal edit tự động nếu navigate từ trang chi tiết đơn hàng
    useEffect(() => {
        const editId = location.state?.editId;
        if (!editId) return;
        // Clear state để tránh re-trigger khi user quay lại
        window.history.replaceState({}, '');
        // Fetch và mở modal
        ordersApi.getById(editId).then(r => {
            const full = r.data;
            setForm({
                customerId: full.customerId,
                channelId: full.channelId,
                orderDate: full.orderDate?.slice(0, 10),
                status: full.status,
                orderDetails: full.orderDetails || [],
            });
            setSelected(full);
            setModal('edit');
    ...
    const openEdit = async (order) => {
        // Fetch full order (kèm orderDetails) TRƯỚC khi mở modal
        // Tránh race condition: user nhấn Lưu trong khi details chưa load xong
        try {
            const r = await ordersApi.getById(order.orderId);
            const full = r.data;
            setForm({
                customerId: full.customerId,
                channelId: full.channelId,
            ...
            setSelected(order);
            setModal('edit');  // mở modal SAU KHI có đủ dữ liệu
        } catch {
    ...
    const save = async () => {
        if (!form.customerId || !form.channelId || !form.orderDate) {
            showToast('Vui lòng điền đầy đủ thông tin', 'error'); return;
        }
        // Chỉ bắt buộc có sản phẩm khi TẠO MỚI, không bắt buộc khi chỉnh sửa
        if (modal === 'create' && form.orderDetails.length === 0) {
            showToast('Đơn hàng phải có ít nhất 1 sản phẩm', 'error'); return;
        }
        setSaving(true);
        try {
            // Backend nhận field "items", không phải "orderDetails"
            const payload = {
            ...
            if (modal === 'create') await ordersApi.create(payload);
            else await ordersApi.update(selected.orderId, payload);
                            ...
                            {orders.map(o => (
                                <tr key={o.orderId} onClick={() => navigate(`/orders/${o.orderId}`)}>
                                    ...
                                    <td onClick={e => e.stopPropagation()} style={{ display: 'flex', gap: 4 }}>
                                        {isStaff && <button className="btn btn-ghost btn-sm" onClick={e => { e.stopPropagation(); openEdit(o); }}>✏</button>}
