                ...
                {loading ? <div className="loading-wrap"><div className="spinner" /><span>Đang tải...</span></div> : (
                    <table>
                        ...
                        <tbody>
                            {items.map((p, i) => (
                                <tr key={p.productId}>
                                    <td className="td-mono">{String(p.productId).padStart(3, '0')}</td>
                                    <td style={{ fontWeight: 600, color: 'var(--text)' }}>{p.productName}</td>
                                    <td><span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 600, background: 'var(--blue-dim)', color: 'var(--blue)' }}>{p.categoryName || '—'}</span></td>
                                    <td style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 13, color: 'var(--green3)', fontWeight: 600 }}>{fmt(p.price)} ₫</td>
                                    <td className="td-dim">{p.createdAt?.slice(0, 10)}</td>
